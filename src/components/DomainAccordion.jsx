...
const DomainAccordion = () => {
  ...
  return (
    <section className="p-4 bg-gray-900">
      <div className="flex flex-col lg:flex-row rounded-xl h-fit lg:h-[450px] w-full max-w-6xl mx-auto shadow overflow-hidden">
        {items.map((item) => {
          return (
            <Panel
              key={item.id}
...
const Panel = ({ open, setOpen, id, Icon, title, imgSrc, description }) => {
  ...
  return (
    <>
      <button
        className="bg-gray-700 hover:bg-slate-50 transition-colors p-3 border-r-[1px] border-b-[1px] border-gray-900 flex flex-row-reverse lg:flex-col justify-end items-center gap-4 relative group"
        onClick={() => setOpen(id)}
      >
        <span
          style={{
        ...
        <span className="w-4 h-4 bg-gray-500 group-hover:bg-slate-50 transition-colors border-r-[1px] border-b-[1px] lg:border-b-0 lg:border-t-[1px] border-slate-200 rotate-45 absolute bottom-0 lg:bottom-[50%] right-[50%] lg:right-0 translate-y-[50%] translate-x-[50%] z-20" />
      </button>
